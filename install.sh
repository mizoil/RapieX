...
REPO="https://api.github.com/repos/mizoil/rapiex/releases/latest"

IS_SHOULD_RESTART_NETWORK=
DOWNLOAD_DIR="/tmp/rapiex"
...
main() {
    ...
    if [ -f "/etc/init.d/rapiex" ]; then
        printf "\033[32;1mRapieX is already installed. Just upgrade it? (y/n)\033[0m\n"
        printf "\033[32;1my - Only upgrade rapiex\033[0m\n"
        ...
        echo "Installed rapiex..."
        add_tunnel
    fi

    opkg install $DOWNLOAD_DIR/rapiex*.ipk
    opkg install $DOWNLOAD_DIR/luci-app-rapiex*.ipk
    ...
    while true; do
        ...
        case $RUS in
        y)
            opkg install $DOWNLOAD_DIR/luci-i18n-rapiex-ru*.ipk
            break
            ;;
    ...
    rm -f $DOWNLOAD_DIR/rapiex*.ipk $DOWNLOAD_DIR/luci-app-rapiex*.ipk $DOWNLOAD_DIR/luci-i18n-rapiex-ru*.ipk
